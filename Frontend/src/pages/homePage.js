...
const HomePage = () => {
    ...
    const { deleteFileFolder, isDeleting } = useDeleteFileFolder();

    const handleDelete = async (elem) => {
        if (window.confirm(`Are you sure you want to delete this ${elem.type}?`)) {
            const success = await deleteFileFolder(elem._id, elem.type);
            if (success) {
                toast.success(`${elem.type} deleted successfully`);
                getFileFolders(parentFolder._id);
            } else {
                toast.error(`Failed to delete ${elem.type}`);
            }
        }
    };
